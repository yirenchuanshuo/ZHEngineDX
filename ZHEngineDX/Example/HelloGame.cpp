...
void HelloGame::OnUpdate(ZHEngineTimer const& timer)
{
	...
	if (g_frameCounter == 500)
	{
		// Update window text with FPS value.
		wchar_t fps[64];
		swprintf_s(fps, L"%ufps", g_timer.GetFramesPerSecond());
...
void HelloGame::LoadPipeline()
{
	...
	ModeShaderPS = std::make_shared<UShader>(L"Shader/Model.hlsl", "PSMain",EShaderType::Pixel);
	ModeShaderVS->CompileShader();
	ModeShaderPS->CompileShader();
	SkyShaderVS = std::make_shared<UShader>(L"Shader/Sky.hlsl", "VSMain", EShaderType::Vertex);
...
void HelloGame::PreperRenderActor()
{
	...
	OBJ Ground;
	Ground.Load(MODEASSETPATH(Ground));
#endif

	ModeActor->LoadMesh(MODEPATH(Cube));
	SkyActor->LoadMesh(MODEPATH(Sky));
	GroundActor->LoadMesh(MODEPATH(Ground));
...
void HelloGame::CreateRootSignature()
{
	...
	GroundActor->SetRootSignature(GetD3DDevice(), rootSignatureDesc);
...
void HelloGame::CreatePSO()
{
	...
	GroundActor->SetPipleLineState(GetD3DDevice(), psoDesc);
...
void HelloGame::UpLoadVertexAndIndexToHeap(std::shared_ptr<RenderActor>& Actor) const
{
...
void HelloGame::UpLoadShaderResource()
{
	...
	for (int i = 0; i < UniformTextureNums; i++)
	{
		...
		GroundCbvSrvHandle.Offset(1, g_cbvsrvDescriptorSize);
		...
		g_device->CreateShaderResourceView(g_Uniformtextures[i].Resource.Get(), &srvDesc, GroundCbvSrvHandle);
...
void HelloGame::LoadTexture()
{
	...
	for (size_t i = 0; i < TextureNums; i++)
	{
		...
		Temptex.texSize = Texture::LoadImageDataFromFile(Temptex.Data, Temptex.texDesc, Temptex.Filename, Temptex.texBytesPerRow);
		Temptex.GenerateTextureData();
		GroundActor->SetTextures(Temptex);
